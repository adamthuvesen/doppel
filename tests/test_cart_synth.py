...
def test_soft_repair_derives_missingness_flag_from_nullable_source() -> None:
    df = pl.DataFrame(
        {
            "days_since_last_activation_l180d": [None, 10, None, 3],
            "days_since_last_activation_missing_l180d": pl.Series(
                "days_since_last_activation_missing_l180d", [0, 1, 0, 0], dtype=pl.Int32
            ),
        }
    )
    out, summary = _repair_output(
        ...
        [
            _MissingFlag(
                "days_since_last_activation_missing_l180d",
                "days_since_last_activation_l180d",
                df["days_since_last_activation_missing_l180d"].dtype,
            )
        ],
        [],
    )
    assert out["days_since_last_activation_missing_l180d"].to_list() == [1, 0, 1, 0]
    assert summary.missing_flags == {"days_since_last_activation_missing_l180d": 3}
    ...
    df = pl.DataFrame(
        {
            "days_since_last_activation_l180d": [None, 10, None, 3],
            "days_since_last_activation_missing_l180d": pl.Series(
                "days_since_last_activation_missing_l180d", [1, 0, 1, 0], dtype=pl.Int32
            ),
        }
    )
...
def test_soft_repair_clips_detected_count_bounds_without_erroring() -> None:
    df = pl.DataFrame(
        {
            "num_free_users": pl.Series("num_free_users", [5, 2, 10], dtype=pl.Int64),
            "num_users": pl.Series("num_users", [3, 2, 8], dtype=pl.Int64),
        }
    )
    out, summary = _repair_output(df, [], [_CountBound("num_free_users", "num_users")])
    assert out["num_free_users"].to_list() == [3, 2, 8]
    assert out["num_free_users"].dtype == pl.Int64
    assert summary.count_bounds == {"num_free_users <= num_users": 2}
